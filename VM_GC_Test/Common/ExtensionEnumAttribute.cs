...
namespace VM_GC_Test.Common
{
    ...
    public static class ExtensionEnumAttributeHelper
    {
        ...
        public static T GetEnumAttributeValue<T>(this Enum pEnum, int valIdx)
        {
            ...
            else if (tObj.ResourceType != null && string.IsNullOrWhiteSpace(tObj.ResourceName) == false)
            {
                PropertyInfo property = tObj.ResourceType.GetProperty(tObj.ResourceName, BindingFlags.Public | BindingFlags.Static);
